...
void DrawData(cv::Mat mm, cv::Mat frame, const std::map<int, DSResult> &map, const std::vector<cv::Rect> &outRcs, bool detect){
...
void CB(cv::Mat &frame, int num, std::string filename){

	if (_vw == NULL) {
		_vw = new cv::VideoWriter(filename, CV_FOURCC('M', 'J', 'P', 'G'), 10.0, cv::Size(frame.cols, frame.rows));
	}
	if (_rcMap.empty()) {
...
int main(int argc, char **argv){
	...
	_imgDir = "/home/aitoe/Documents/RonakDataset/ATM/test/clip2/img1/";
	_imgDir = argv[2];
	// _rcFile = "/home/aitoe/Documents/RonakDataset/ATM/test/clip2/gt/gt.txt";
	_rcFile = "/home/aitoe/AitoeLabs/ATM/detections/oifinal_0.05.txt";
	_rcFile = argv[3];
	...
	_imgCount = toInt(argv[4]);// 2001;// 750;// 680;
	std::cout << argv[5] << '\n';
	Go(argv[5]);
	return 0;
}
