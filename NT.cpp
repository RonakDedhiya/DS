...
// 			rc = lr;
// 		}
// 		std::cout << "in extract faeture" << rc.x << " " << rc.y << " " << rc.width << " " << rc.height  << '\n';
//
...
// 	maxh += 10;
	...
	FeatureGetter::Instance()->Get(in, rcsin, newfts);
	for(int i = 0; i < rcsin.size(); i++){
...
NT::NT(){
	tt_ = TTrackerP(new TTracker(0.7, 30, 3));
}
...
#endif
	// if(0){// just a test
	// 	Mat frame = cv::imread("/home/aitoe/Documents/RonakDataset/ATM/test/clip2/img1/000001.png");
	// 	Mat nnn;
...
// 	#endif
// #endif
	NearestNeighborDistanceMetric::Instance()->Init(0.35, 100);
